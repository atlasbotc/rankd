...
// Verified against Marqui/Theme/DesignSystem.swift values:
...
struct MarquiTimelineProvider: TimelineProvider {
    typealias Entry = MarquiWidgetEntry
    
    func placeholder(in context: Context) -> MarquiWidgetEntry {
        MarquiWidgetEntry(date: Date(), items: sampleItems)
    }
    
    func getSnapshot(in context: Context, completion: @escaping (MarquiWidgetEntry) -> Void) {
        let items = loadItems()
        completion(MarquiWidgetEntry(date: Date(), items: items.isEmpty ? sampleItems : items))
    }
    
    func getTimeline(in context: Context, completion: @escaping (Timeline<MarquiWidgetEntry>) -> Void) {
        let items = loadItems()
        let entry = MarquiWidgetEntry(date: Date(), items: items)
...
struct MarquiWidgetEntry: TimelineEntry {
    let date: Date
...
struct MarquiSmallView: View {
    let entry: MarquiWidgetEntry
...
struct MarquiWidgetEntryView: View {
    @Environment(\.widgetFamily) var family
    var entry: MarquiTimelineProvider.Entry
    
    var body: some View {
        Group {
            switch family {
            case .systemSmall:
                MarquiSmallView(entry: entry)
            case .systemMedium:
                MarquiMediumView(entry: entry)
            case .systemLarge:
                MarquiLargeView(entry: entry)
            default:
                MarquiSmallView(entry: entry)
            }
        }
...
struct MarquiWidget: Widget {
    let kind: String = "MarquiWidget"
    
    var body: some WidgetConfiguration {
        StaticConfiguration(kind: kind, provider: MarquiTimelineProvider()) { entry in
            if #available(iOS 17.0, *) {
                MarquiWidgetEntryView(entry: entry)
...
#Preview("Small", as: .systemSmall) {
    ...
    MarquiWidgetEntry(date: Date(), items: [
        WidgetItem(id: "1", title: "The Shawshank Redemption", score: 9.8, tier: "Good", posterURL: nil, rank: 1)
    ])
}
